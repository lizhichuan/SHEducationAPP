package com.njcool.lzccommon.network.http.config;


import com.njcool.lzccommon.network.common.ViseConfig;
import com.njcool.lzccommon.network.http.CoolHttp;
import com.njcool.lzccommon.network.http.core.ApiCookie;
import com.njcool.lzccommon.network.http.interceptor.GzipRequestInterceptor;
import com.njcool.lzccommon.network.http.interceptor.OfflineCacheInterceptor;
import com.njcool.lzccommon.network.http.interceptor.OnlineCacheInterceptor;
...
public class HttpGlobalConfig {
    ...
    private boolean isCookie = true;//是否使用Cookie
    ...
    public HttpGlobalConfig proxy(Proxy proxy) {
        CoolHttp.getOkHttpBuilder().proxy(checkNotNull(proxy, "proxy == null"));
        return this;
    }
    ...
    public HttpGlobalConfig connectTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            CoolHttp.getOkHttpBuilder().connectTimeout(timeout, unit);
        } else {
            CoolHttp.getOkHttpBuilder().connectTimeout(ViseConfig.DEFAULT_TIMEOUT, TimeUnit.SECONDS);
        }
        return this;
    }
    ...
    public HttpGlobalConfig writeTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            CoolHttp.getOkHttpBuilder().writeTimeout(timeout, unit);
        } else {
            CoolHttp.getOkHttpBuilder().writeTimeout(ViseConfig.DEFAULT_TIMEOUT, TimeUnit.SECONDS);
        }
        return this;
    }
    ...
    public HttpGlobalConfig readTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            CoolHttp.getOkHttpBuilder().readTimeout(timeout, unit);
        } else {
            CoolHttp.getOkHttpBuilder().readTimeout(ViseConfig.DEFAULT_TIMEOUT, TimeUnit.SECONDS);
        }
        return this;
    }
    ...
    public HttpGlobalConfig interceptor(Interceptor interceptor) {
        CoolHttp.getOkHttpBuilder().addInterceptor(checkNotNull(interceptor, "interceptor == null"));
        return this;
    }
    ...
    public HttpGlobalConfig networkInterceptor(Interceptor interceptor) {
        CoolHttp.getOkHttpBuilder().addNetworkInterceptor(checkNotNull(interceptor, "interceptor == null"));
        return this;
    }
    ...
    public HttpGlobalConfig cacheOffline(Cache httpCache) {
        networkInterceptor(new OfflineCacheInterceptor(CoolHttp.getContext()));
        interceptor(new OfflineCacheInterceptor(CoolHttp.getContext()));
        this.httpCache = httpCache;
        return this;
    }
    ...
    public HttpGlobalConfig cacheOffline(Cache httpCache, final int cacheControlValue) {
        networkInterceptor(new OfflineCacheInterceptor(CoolHttp.getContext(), cacheControlValue));
        interceptor(new OfflineCacheInterceptor(CoolHttp.getContext(), cacheControlValue));
        this.httpCache = httpCache;
        return this;
    }
