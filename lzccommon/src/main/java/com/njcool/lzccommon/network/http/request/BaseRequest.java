...
import com.njcool.lzccommon.common.CookieManger2;
import com.njcool.lzccommon.network.common.ViseConfig;
import com.njcool.lzccommon.network.http.CoolHttp;
import com.njcool.lzccommon.network.http.callback.UCallback;
import com.njcool.lzccommon.network.http.config.HttpGlobalConfig;
import com.njcool.lzccommon.network.http.interceptor.HeadersInterceptor;
import com.njcool.lzccommon.network.http.interceptor.UploadProgressInterceptor;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    protected void generateLocalConfig() {
        OkHttpClient.Builder newBuilder = CoolHttp.getOkHttpClient().newBuilder();
        ...
        if (baseUrl != null) {
            ...
            CoolHttp.getRetrofitBuilder().client(newBuilder.build());
            retrofit = CoolHttp.getRetrofitBuilder().build();
        }
    }
    ...
    protected void generateGlobalConfig() {
        httpGlobalConfig = CoolHttp.CONFIG();

        if (httpGlobalConfig.getBaseUrl() == null) {
            httpGlobalConfig.baseUrl(ApiHost.getHost());
        }
        CoolHttp.getRetrofitBuilder().baseUrl(httpGlobalConfig.getBaseUrl());

        if (httpGlobalConfig.getConverterFactory() != null) {
            CoolHttp.getRetrofitBuilder().addConverterFactory(httpGlobalConfig.getConverterFactory());
        }

        if (httpGlobalConfig.getCallAdapterFactory() == null) {
            httpGlobalConfig.callAdapterFactory(RxJava2CallAdapterFactory.create());
        }
        CoolHttp.getRetrofitBuilder().addCallAdapterFactory(httpGlobalConfig.getCallAdapterFactory());

        if (httpGlobalConfig.getCallFactory() != null) {
            CoolHttp.getRetrofitBuilder().callFactory(httpGlobalConfig.getCallFactory());
        }
        ...
        CoolHttp.getOkHttpBuilder().hostnameVerifier(httpGlobalConfig.getHostnameVerifier());

        if (httpGlobalConfig.getSslSocketFactory() == null) {
            httpGlobalConfig.SSLSocketFactory(SSLUtil.getSslSocketFactory(null, null, null));
        }
        CoolHttp.getOkHttpBuilder().sslSocketFactory(httpGlobalConfig.getSslSocketFactory());
        ...
        CoolHttp.getOkHttpBuilder().connectionPool(httpGlobalConfig.getConnectionPool());

//        if (httpGlobalConfig.isCookie() && httpGlobalConfig.getApiCookie() == null) {
//            httpGlobalConfig.apiCookie(new ApiCookie(CoolHttp.getContext()));
//        }
        if (httpGlobalConfig.isCookie()) {
//            CoolHttp.getOkHttpBuilder().cookieJar(httpGlobalConfig.getApiCookie());
            CoolHttp.getOkHttpBuilder().cookieJar(new CookieManger2(CoolHttp.getContext()));
        }

        if (httpGlobalConfig.getHttpCacheDirectory() == null) {
            httpGlobalConfig.setHttpCacheDirectory(new File(CoolHttp.getContext().getCacheDir(), ViseConfig.CACHE_HTTP_DIR));
        }
        ...
        if (httpGlobalConfig.getHttpCache() != null) {
            CoolHttp.getOkHttpBuilder().cache(httpGlobalConfig.getHttpCache());
        }
    }
