...
public abstract class BaseHttpRequest<R extends BaseHttpRequest> extends BaseRequest<R> {
    ...
    protected void generateLocalConfig() {
        ...
        if (isLocalCache) {
            if (cacheKey != null) {
                CoolHttp.getApiCacheBuilder().cacheKey(cacheKey);
            } else {
                CoolHttp.getApiCacheBuilder().cacheKey(ApiHost.getHost());
            }
            if (cacheTime > 0) {
                CoolHttp.getApiCacheBuilder().cacheTime(cacheTime);
            } else {
                CoolHttp.getApiCacheBuilder().cacheTime(ViseConfig.CACHE_NEVER_EXPIRE);
            }
        }
        if (baseUrl != null && isLocalCache && cacheKey == null) {
            CoolHttp.getApiCacheBuilder().cacheKey(baseUrl);
        }
