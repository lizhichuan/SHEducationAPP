...
public class CourseAllFragment extends BaseFragment {
    ...
    private CoolCommonRecycleviewAdapter<GetStudyListResultVO.ResultBean.DataBean> adapter;
    private List<GetStudyListResultVO.ResultBean.DataBean> mDatas;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, final Bundle savedInstanceState) {
        ...
        adapter = new CoolCommonRecycleviewAdapter<GetStudyListResultVO.ResultBean.DataBean>(mDatas, getActivity(),
    ...
    private void GetStudyList(boolean refresh) {
        ...
        GetAllStudyList(getActivity(), CoolSPUtil.getDataFromLoacl(getActivity(), "labels"), "", page, 15);
    }
    ...
    public void setData(GetStudyListResultVO.ResultBean getStudyListVO) {
        swp.setRefreshing(false);
    ...
    private void GetAllStudyList(final Context context, String labels, String q, int page, int pagesize) {
        if (!CoolPublicMethod.isNetworkAvailable(context)) {
            CoolPublicMethod.Toast(context, AYConsts.NETWOEKERROR);
            return;
        } else {
            Call<GetStudyListResultVO> call = CoolHttpUrl.getApi().getService().GetStudyList(labels, q, page, pagesize);
            call.enqueue(new Callback<GetStudyListResultVO>() {
                @Override
                public void onResponse(Call<GetStudyListResultVO> call, Response<GetStudyListResultVO> response) {
                    CoolPublicMethod.hideProgressDialog();
                    if (response.isSuccessful()) {
                        GetStudyListResultVO data = response.body();
                        CoolLogTrace.e("GetStudyList", "", GsonUtil.gson().toJson(data).toString());
                ...
                @Override
                public void onFailure(Call<GetStudyListResultVO> call, Throwable t) {
                    CoolPublicMethod.hideProgressDialog();
